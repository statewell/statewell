import { createTaskTables, taskOperation, taskSchemas } from "./tasks.ts";
...
try {
    ...
    try {
      ...
      try {
        ...
        if (!file.isFile() || file.nlink !== 1 || readSync(fd, header, 0, 100, 0) !== 100 ||
            ...
            header.readUInt32BE(60) !== 4 || header.readUInt32BE(68) !== 0x5354574c) throw new Error();
      ...
      if (version.user_version !== 4) throw new Error();
      inspection.query("SELECT project_id, id, revision, contract_revision, state FROM tasks LIMIT 0").all();
      inspection.query("SELECT project_id, task_id, revision, contract_revision, content, transition, continuation FROM checkpoints LIMIT 0").all();
  ...
  if (!existed) {
    db.transaction(() => {
      ...
      createTaskTables(db);
      db.exec("PRAGMA user_version=4; PRAGMA application_id=1398036300");
  ...
  parentPort!.on("message", (request) => {
    try {
      ...
      else if (Object.hasOwn(taskSchemas, request.operation)) value = taskOperation(db, directory, request.operation, request.input);
