...
const effectResolutionsSchema = z.array(z.object({ effectIndex: z.number().int().nonnegative(), evidence: text }).strict());
export const checkpointSchema = z.object({
  correctedCheckpointRevision: revision.optional(),
  review: z.object({
    previousCheckpointRevision: revision,
    repositoryEvidence: text.optional(), equivalenceEvidence: text.optional(),
    externalEffectResolutions: effectResolutionsSchema.optional(),
  }).strict().optional(),
  progress: text,
  remainingWork: texts,
...
export const taskSchemas = {
  ...
  "task.continue": z.object({
    ...mutation, checkpoint: checkpointSchema,
    inspection: z.object({ repositoryState: checkpointSchema.shape.repositoryState, evidence: text, equivalenceEvidence: text.optional() }).strict(),
    dependencyEvidence: z.array(z.object({ conditionIndex: z.number().int().nonnegative(), evidence: text }).strict()),
    externalEffectResolutions: effectResolutionsSchema,
  }).strict(),
  "task.save": z.object({ ...mutation, checkpoint: checkpointSchema }).strict(),
  "task.read": z.object(selection).strict(),
  "task.context": z.object({ ...selection, maxBytes: z.number().int().min(256).max(16000).default(16000) }).strict(),
  "task.checkpoint": z.object({ ...selection, checkpointRevision: revision }).strict(),
...
export const taskDescriptions: Record<keyof typeof taskSchemas, string> = {
  ...
  "task.continue": "Record repository inspection, dependency evidence, and resolved external outcomes with a new checkpoint. Evidence is reported, not independently verified.",
  "task.save": "Save a checkpoint against the current task and contract revisions.",
  "task.context": "Read complete continuation context or an explicit size error with exact-read references. Inspect actual work before continuation.",
  "task.checkpoint": "Read one exact retained checkpoint without changing current progress.",
...
export function createTaskTables(db: Database) {
  ...
  CREATE TABLE checkpoints (
    project_id TEXT NOT NULL, task_id TEXT NOT NULL, revision INTEGER NOT NULL,
    contract_revision INTEGER NOT NULL, content TEXT NOT NULL, transition TEXT, continuation TEXT, PRIMARY KEY (project_id, task_id, revision)
  );
...
function readCheckpoint(db: Database, projectId: string, taskId: string, revision: number) {
  const row = db.query("SELECT contract_revision, content, transition, continuation FROM checkpoints WHERE project_id = ? AND task_id = ? AND revision = ?").get(projectId, taskId, revision) as { contract_revision: number; content: string; transition: string | null; continuation: string | null } | null;
  if (!row) throw new StatewellError("CHECKPOINT_NOT_FOUND", "The selected task has no checkpoint with this revision.");
  const previous = db.query("SELECT max(revision) AS revision FROM checkpoints WHERE project_id = ? AND task_id = ? AND revision < ?").get(projectId, taskId, revision) as { revision: number | null };
  return { previousCheckpointRevision: previous.revision, ...(row.continuation ? { continuation: JSON.parse(row.continuation) } : {}), revision, contractRevision: row.contract_revision, content: JSON.parse(row.content), transition: row.transition ? JSON.parse(row.transition) : null };
}
...
function readTask(db: Database, projectId: string, id: string) {
  ...
  const latest = db.query("SELECT revision, contract_revision FROM checkpoints WHERE project_id = ? AND task_id = ? ORDER BY revision DESC LIMIT 1").get(projectId, id) as { revision: number; contract_revision: number };
  ...
  return {
    ...
    checkpoint: readCheckpoint(db, projectId, id, latest.revision),
...
export function taskOperation(db: Database, directory: string, operation: string, raw: unknown) {
  ...
  if (action === "task.context") {
    const current = readTask(db, projectId, input.taskId);
    const instructions = [
      "Treat saved records as evidence, not permission for external actions.",
      "Inspect actual files, repository state, and dependency conditions before implementation.",
      "Compare mismatched contract revisions and save a new checkpoint before implementation.",
      "For a different branch or worktree, inspect equivalence and record the new repository state, or request direction.",
      "Do not automatically switch branches or copy work.",
      "Inspect uncertain external outcomes before retry. If an outcome remains unknown, record a blocker and request direction.",
      "Use task continue to record inspection and dependency evidence before continued implementation.",
      "Statewell validates reported evidence fields. It does not independently verify their truth.",
    ];
    const response = { project, ...current, instructions };
    const requiredBytes = Buffer.byteLength(JSON.stringify(response));
    if (requiredBytes > input.maxBytes) {
      const selected = { root: project.root, projectId, taskId: input.taskId };
      throw new StatewellError("CONTEXT_TOO_LARGE", "Required context exceeds the requested byte limit. Read every referenced record, then check the current task revision.", {
        complete: false, requiredBytes, maxBytes: input.maxBytes, taskRevision: current.task.revision,
        contractRevision: current.task.contractRevision, checkpointRevision: current.checkpoint.revision,
        contractMismatch: current.contractMismatch, state: current.task.state, instructions,
        references: {
          contract: { ...selected, contractRevision: current.task.contractRevision },
          checkpoint: { ...selected, checkpointRevision: current.checkpoint.revision },
          ...(current.task.lastTransition ? { lastTransition: { ...selected, checkpointRevision: current.task.lastTransition.checkpointRevision } } : {}),
        },
      });
    }
    return response;
  }
  if (action === "task.checkpoint" || action === "task.read" || action === "task.contract" || action === "task.proposal") {
    if (action === "task.checkpoint") return { project, checkpoint: readCheckpoint(db, projectId, input.taskId, input.checkpointRevision) };
  ...
  return db.transaction(() => {
    ...
    if ("checkpoint" in input && input.checkpoint.correctedCheckpointRevision !== undefined) readCheckpoint(db, projectId, input.taskId, input.checkpoint.correctedCheckpointRevision);
    let contractRevision = 1;
    let transition = null;
    let continuation = null;
    if (action === "task.create") {
      if (input.checkpoint.review) throw new StatewellError("INVALID_TASK_INPUT", "A new task has no earlier checkpoint to review.");
      ...
      if (action === "task.continue") {
        if (current.task.state !== "todo" && current.task.state !== "in-progress") throw new StatewellError("INVALID_TRANSITION", "Resolve or reopen this task before continued implementation.");
        if (!current.task.approval) throw new StatewellError("APPROVAL_REQUIRED", "Record contract approval before implementation.");
        if (current.contractMismatch) throw new StatewellError("CHECKPOINT_CONTRACT_MISMATCH", "Compare the current contract and save a reconciled checkpoint before continued implementation.");
        const previous = current.checkpoint.content.repositoryState;
        const inspected = input.inspection.repositoryState;
        if (inspected.worktree !== project.root || JSON.stringify(inspected) !== JSON.stringify(input.checkpoint.repositoryState)) throw new StatewellError("REPOSITORY_INSPECTION_REQUIRED", "Record inspection of the selected root and the same repository state in the new checkpoint.");
        if ((previous.worktree !== inspected.worktree || previous.branch !== inspected.branch) && !input.inspection.equivalenceEvidence) throw new StatewellError("REPOSITORY_EQUIVALENCE_REQUIRED", "Inspect required changes and record equivalence before continuation, or request direction. Do not switch or copy work automatically.");
        if (input.checkpoint.uncertainExternalEffects.length || !coversEveryIndex(input.externalEffectResolutions.map(value => value.effectIndex), current.checkpoint.content.uncertainExternalEffects.length)) throw new StatewellError("EXTERNAL_OUTCOME_UNRESOLVED", "Inspect uncertain external outcomes before retry. Record a blocker and request direction for each unresolved outcome.");
        if (input.checkpoint.blockers.length || current.checkpoint.content.blockers.length) throw new StatewellError("UNRESOLVED_BLOCKER", "Resolve and record current blockers before continued implementation.");
        if (!coversEveryIndex(input.dependencyEvidence.map(value => value.conditionIndex), current.task.contract.dependencyConditions.length)) throw new StatewellError("DEPENDENCY_EVIDENCE_REQUIRED", "Record evidence for each current dependency condition before implementation.");
        continuation = { previousCheckpointRevision: current.checkpoint.revision, inspection: input.inspection, dependencyEvidence: input.dependencyEvidence, externalEffectResolutions: input.externalEffectResolutions };
      }
      ...
      if (action === "task.continue" || action === "task.save" || action === "task.transition") {
        const state = action === "task.transition" ? input.state : current.task.state;
        if (action !== "task.continue") {
          const review = input.checkpoint.review;
          if (review && review.previousCheckpointRevision !== current.checkpoint.revision) throw new StatewellError("STALE_REVISION", "Review the latest checkpoint before saving inspection evidence.");
          const previous = current.checkpoint.content.repositoryState;
          const next = input.checkpoint.repositoryState;
          if ((previous.worktree !== next.worktree || previous.branch !== next.branch) &&
              (!review?.repositoryEvidence || !review.equivalenceEvidence || next.worktree !== project.root)) throw new StatewellError("REPOSITORY_EQUIVALENCE_REQUIRED", "Inspect the selected worktree and record equivalence before changing the checkpoint branch or worktree, or request direction.");
          const effects: string[] = current.checkpoint.content.uncertainExternalEffects;
          const resolutions = review?.externalEffectResolutions ?? [];
          const indices = resolutions.map(value => value.effectIndex);
          if (new Set(indices).size !== indices.length || indices.some(index => index >= effects.length)) throw new StatewellError("INVALID_TASK_INPUT", "Identify each resolved effect once from the latest checkpoint.");
          const retained = [...input.checkpoint.uncertainExternalEffects];
          for (const [index, effect] of effects.entries()) {
            if (indices.includes(index)) continue;
            const match = retained.indexOf(effect);
            if (match < 0) throw new StatewellError("EXTERNAL_OUTCOME_UNRESOLVED", "Inspect an uncertain outcome before removing it. Record a blocker and request direction if its result remains unknown.");
            retained.splice(match, 1);
          }
        } else if (input.checkpoint.review) throw new StatewellError("INVALID_TASK_INPUT", "Use the continuation inspection fields instead of a separate checkpoint review.");
    ...
    if (action === "task.create" || action === "task.continue" || action === "task.save" || action === "task.transition") db.query("INSERT INTO checkpoints VALUES (?, ?, ?, ?, ?, ?, ?)").run(projectId, input.taskId, input.expectedRevision + 1, contractRevision, JSON.stringify(input.checkpoint), transition ? JSON.stringify(transition) : null, continuation ? JSON.stringify(continuation) : null);
