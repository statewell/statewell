...
test("MCP initializes and shares selected instance projects with CLI", async () => {
  ...
  try {
    ...
    expect((await client.listTools()).tools.map(tool => tool.name).sort()).toEqual(["instance_inspect", "project_inspect", "project_register", "project_resolve", "task_approve", "task_check", "task_contract", "task_create", "task_proposal", "task_propose", "task_read", "task_save"]);
